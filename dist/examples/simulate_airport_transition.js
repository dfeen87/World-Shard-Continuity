...
async function seedIdentity(store, actor) {
...
function seedAsset(ledger, ownerPid, scope, world, shard) {
...
async function printAsset(ledger, aid, label) {
    const a = await ledger.get(aid);
    if (!a)
        throw new Error(`Missing asset ${aid}`);
    console.log(`${label}: asset_id=${a.asset_id} status=${a.state.status} owner=${a.owner.owner_id} v=${a.integrity.version ?? 1}`);
}
async function happyPath(fsm, ledger, actor, pid, protectedAssets) {
    console.log("\n=== HAPPY PATH ===");
    ...
    for (const aid of protectedAssets)
        await printAsset(ledger, aid, "After prepare (escrow expected)");
    ...
    for (const aid of protectedAssets)
        await printAsset(ledger, aid, "After confirm (active expected)");
}
async function destinationFailureRollback(fsm, ledger, actor, pid, protectedAssets) {
    ...
    for (const aid of protectedAssets)
        await printAsset(ledger, aid, "After rollback (active expected)");
}
async function idempotencyChecks(fsm, ledger, actor, pid, protectedAssets) {
    console.log("\n=== IDEMPOTENCY CHECKS ===");
    const fromShard = "sid_origin";
    const toShard = "sid_destination";
    ...
    for (const aid of protectedAssets)
        await printAsset(ledger, aid, "After cleanup rollback (active expected)");
}
async function main() {
