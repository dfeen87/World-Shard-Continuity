...
async function main() {
  ...
  ledger.seed({
    ...
    asset_id: aid,
    asset_class: "item",
    asset_type: "ranked_loadout",
    scope: "global",
    owner: { owner_type: "player", owner_id: pid },
    state: { status: "active", quantity: 1, attributes: { label: "Loadout" } },
  ...
  const request = {
    kind: "matchmaking_queue" as const,
    identity_id: pid,
    from_shard: "sid_hub",
    to_shard: "sid_match_777",
    protected_assets: [aid],
    metadata: { match_id: `mid_${Date.now()}` }
  };

  const request_id = "req_mm_ranked_777";
  const a = await executeTransition(ctx, registry, idemp, {
    action: "begin",
    request_id,
    change_id: "chg_mm_1",
    request
  ...
  const b = await executeTransition(ctx, registry, idemp, {
    action: "begin",
    request_id,
    change_id: "chg_mm_2",
    request
  ...
  assert(a.transition!.transition_id === b.transition!.transition_id, "Retry must return same transition_id.");
  console.log("✅ request_id idempotency verified (matchmaking)");
}
